...
export function PlayerLevelPeSection({
  ...
  const setAmount = (value: number) => {
    setPeCount(Math.max(1, Math.min(availablePe, value)));
    setStep("idle");
  };

  const adjustPe = (delta: number) => {
    setAmount(peCount + delta);
  };
            ...
            <div className="flex flex-wrap gap-1.5">
              {[5, 10].map((n) => (
                <button
                  ...
                  onClick={() => adjustPe(n)}
                  disabled={peCount >= availablePe || isProcessing || step === "confirm"}
                  className="px-2.5 py-1 rounded-lg border border-slate-700 bg-slate-900/60 text-slate-400 hover:border-violet-700/50 hover:text-violet-300 text-[10px] font-black uppercase tracking-wider transition-colors disabled:opacity-40"
                >
                  +{n}
                </button>
              ))}
              <button
                type="button"
                onClick={() => setAmount(availablePe)}
                disabled={isProcessing || step === "confirm" || peCount >= availablePe}
                className="px-2.5 py-1 rounded-lg border border-slate-700 bg-slate-900/60 text-slate-400 hover:border-violet-700/50 hover:text-violet-300 text-[10px] font-black uppercase tracking-wider transition-colors disabled:opacity-40"
              >
                Máx
              </button>
